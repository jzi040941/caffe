...
template <typename Dtype>
class DLPrimConvolutionLayerTest : public GPUDeviceTest<Dtype> {
 protected:
  DLPrimConvolutionLayerTest()
      : blob_bottom_(new Blob<Dtype>(2, 3, 6, 4)),
        blob_bottom_2_(new Blob<Dtype>(2, 3, 6, 4)),
        blob_top_(new Blob<Dtype>()),
        blob_top_2_(new Blob<Dtype>()) {}
  ...
  virtual ~DLPrimConvolutionLayerTest() {
    delete blob_bottom_;
    delete blob_bottom_2_;
    delete blob_top_;
    delete blob_top_2_;
...
TYPED_TEST_CASE(DLPrimConvolutionLayerTest, TestDtypes);

TYPED_TEST(DLPrimConvolutionLayerTest, TestSetupDLPrim) {
  if (Caffe::GetDefaultDevice()->backend() == BACKEND_OpenCL && sizeof(TypeParam) == 4) {
    this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
    this->blob_top_vec_.push_back(this->blob_top_2_);
    LayerParameter layer_param;
    ConvolutionParameter* convolution_param =
    ...
    shared_ptr<Layer<TypeParam> > layer(
        new DLPrimConvolutionLayer<TypeParam>(layer_param));
    layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
    EXPECT_EQ(this->blob_top_->num(), 2);
    EXPECT_EQ(this->blob_top_->channels(), 4);
    EXPECT_EQ(this->blob_top_->height(), 2);
    ...
    convolution_param->set_num_output(3);
    convolution_param->set_group(3);
    layer.reset(new DLPrimConvolutionLayer<TypeParam>(layer_param));
    layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
    EXPECT_EQ(this->blob_top_->num(), 2);
    EXPECT_EQ(this->blob_top_->channels(), 3);
    EXPECT_EQ(this->blob_top_->height(), 2);
...
TYPED_TEST(DLPrimConvolutionLayerTest, TestSimpleConvolutionDLPrim) {
  if (Caffe::GetDefaultDevice()->backend() == BACKEND_OpenCL && sizeof(TypeParam) == 4) {
    this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
    this->blob_top_vec_.push_back(this->blob_top_2_);
    LayerParameter layer_param;
...
TYPED_TEST(DLPrimConvolutionLayerTest, TestSimpleConvolutionGroupDLPrim) {
  if (Caffe::GetDefaultDevice()->backend() == BACKEND_OpenCL && sizeof(TypeParam) == 4) {
    LayerParameter layer_param;
    ConvolutionParameter* convolution_param =
        layer_param.mutable_convolution_param();
...
TYPED_TEST(DLPrimConvolutionLayerTest, TestSobelConvolutionDLPrim) {
  if (Caffe::GetDefaultDevice()->backend() == BACKEND_OpenCL && sizeof(TypeParam) == 4 ) {
      // Test separable convolution by computing the Sobel operator
      // as a single filter then comparing the result
      // as the convolution of two rectangular filters.
...
TYPED_TEST(DLPrimConvolutionLayerTest, TestGradientDLPrim) {
  if (Caffe::GetDefaultDevice()->backend() == BACKEND_OpenCL && sizeof(TypeParam) == 4 ) {
    LayerParameter layer_param;
    ConvolutionParameter* convolution_param =
        layer_param.mutable_convolution_param();
    ...
    convolution_param->mutable_weight_filler()->set_type("gaussian");
    convolution_param->mutable_bias_filler()->set_type("gaussian");
    DLPrimConvolutionLayer<TypeParam> layer(layer_param);
...
TYPED_TEST(DLPrimConvolutionLayerTest, TestGradientGroupDLPrim) {
  if (Caffe::GetDefaultDevice()->backend() == BACKEND_OpenCL && sizeof(TypeParam) == 4 ) {
    LayerParameter layer_param;
    ConvolutionParameter* convolution_param =
        layer_param.mutable_convolution_param();
